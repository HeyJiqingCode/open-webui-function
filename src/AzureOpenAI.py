...
class Pipe:
    ...
    async def pipe(
        ...
        # 替换URL中的<model-name>占位符
        endpoint_url = self.valves.AZURE_AI_ENDPOINT
        if "<model-name>" in endpoint_url and selected_model:
            endpoint_url = endpoint_url.replace("<model-name>", selected_model)
        elif "<model-name>" in endpoint_url and self.valves.AZURE_AI_MODEL:
            # 如果有默认模型配置且URL中有占位符，使用默认模型中的第一个
            models = self.parse_models(self.valves.AZURE_AI_MODEL)
            if models and len(models) > 0:
                endpoint_url = endpoint_url.replace("<model-name>", models[0])
